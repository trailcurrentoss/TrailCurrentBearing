#!/bin/bash
# Build Bearing firmware and produce both output formats.
#   build/bearing.bin         — app-only (for OTA via Headwaters)
#   build/bearing_merged.bin  — merged  (for web flasher, full flash at 0x0)
#
...
echo "========================================"
echo "Building Bearing ..."
echo "========================================"
idf.py build

# Create merged binary (for web flasher — flashable at 0x0)
esptool.py --chip esp32s3 merge_bin -o "$OUTPUT_DIR/bearing_merged.bin" \
    ...
    0x10000 "$OUTPUT_DIR/bearing.bin"
...
ls -lh "$OUTPUT_DIR/bearing.bin"
echo ""
echo "Merged binary (for web flasher):"
ls -lh "$OUTPUT_DIR/bearing_merged.bin"
echo ""
